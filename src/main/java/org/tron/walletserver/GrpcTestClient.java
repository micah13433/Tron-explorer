...
//  }
  ...
  public GrpcTestClient(String fullnode, String soliditynode) {
    if (!StringUtils.isEmpty(fullnode)) {
      channelFull = NettyChannelBuilder.forTarget(fullnode)
...
//  }
  ...
  public long getLastestConfirmedBlockByLatestNum() {
    EmptyMessage emptyMessage = EmptyMessage.newBuilder().build();
    Block block = blockingStubSolidity.getNowBlock(emptyMessage);
