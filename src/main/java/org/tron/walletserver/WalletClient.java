...
//  }
  ...
  public static boolean addressValid(byte[] address) {
    ...
    if (address.length != Constrain.ADDRESS_SIZE) {
      logger.warn("Warning: Address length need " + Constrain.ADDRESS_SIZE + " but " + address.length
              + " !!");
      return false;
    }
    byte preFixbyte = address[0];
    if (preFixbyte != Constrain.ADD_PRE_FIX_BYTE) {
      logger.warn("Warning: Address need prefix with " + Constrain.ADD_PRE_FIX_BYTE + " but "
          + preFixbyte + " !!");
      return false;
    }
    //Other rule;
  ...
  public static byte[] decodeFromBase58Check(String addressBase58) {
    ...
    if (addressBase58.length() != Constrain.BASE58CHECK_ADDRESS_SIZE) {
      logger.warn("Warning: Base58 address length need " + Constrain.BASE58CHECK_ADDRESS_SIZE
...
//  }
  ...
  public static Transaction freezeBalance(FreezeBalanceContract contract) {
    Transaction transaction = rpcCli.createTransaction(contract);
    return transaction;
  }
